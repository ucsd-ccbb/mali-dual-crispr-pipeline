...
# Dummy definition just to make RStudio stop complaining that this function name isn't defined in scope:
saveWorkspaceVariables = NULL
# In reality, this name gets (re)defined by the source import below
...
plot_fit <- function(x1, ac1, fc1, ab1, x2, ac2, fc2, ab2, minfc = 0.10, nt, pA_pB) {
  nx <- nrow(x1)
  maxt <- time[nt] + 3
...
irls <- function(fc, w0, probes, ag = 2,
                 tol = 1e-3, maxiter = 30, small) {
...
plotOverlappingHist <- function(a, b, colors = c("gray70", "gray20", "gray50"),
                                breaks = NULL, xlim = NULL, ylim = NULL, xlab = NULL,
                                ylab = NULL, main = NULL) {
  ahist = NULL
  bhist = NULL
...
plot_scatterplots <- function(nn, nt, a1, a2, fc, bad1, bad2, x1, x2, good) {
...
plotFcHists <- function(useSeed, nn, pp_fc, fc, allbad) {
...
writeLogFreqVsTimePlots <- function(outputDir, project, x1, a1, fc, ab1, x2, a2, ab2, nt, pA_pB) {
  pdf(file.path(outputDir, paste(project, ".pdf", sep = "")))
  par(pty = "s", mfrow = c(1, 1))
  plot_fit(x1, a1, fc, ab1, x2, a2, fc, ab2, minfc = 0.10, nt, pA_pB)
  dev.off()
}

plotConstructFitnessesHist <- function(bad1, bad2, fc, allbad) {
  rge <- range(fc)
...
writeSingleGeneFitnessesFile <- function(outputDir, project, genes, f, f_sd) {
  ...
  write.table(
    ...
    file = file.path(outputDir, paste(project, "_f.txt", sep = "")),
    sep = "\t",
    row.names = FALSE,
    quote = FALSE
  )
...
plotPiScoreHist <- function(pi_mean, pi_null) {
  rge <- range(pi_mean)
...
writeConstructFitnessesFile <- function(outputDir, project, pA_pB, fc, sdfc) {
  ...
  # TODO: refactor file suffix, separator into easier to manage location
  write.table(format(resp, digits = 7), file = file.path(outputDir, paste(project, "_fc.txt", sep = "")),
              sep = "\t", row.names = FALSE,
              quote = FALSE
  )
}
...
if (gRun) {
  ...
  plotConstructFitnessesHist(bad1, bad2, fc, allbad)
